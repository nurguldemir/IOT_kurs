...
#define wifi_ssid "G4_1299"
#define wifi_password "nunununu"
#define mqtt_server "m10.cloudmqtt.com"
#define mqtt_user "eyexuxtd"
#define mqtt_password "LwqIZA_-Ag3w"
#define topic0 "uygulama3"
#define topic1 "uygulama3_1"
#define topic2 "uygulama3_2"
...
PubSubClient client(mqtt_server, 11862, callback, espClient);
...
void callback(char* topic, byte* payload, unsigned int length) {
  char rxBuffer[readSize]; 
 //Gelen datayı okumak için bir Buffer dizisi tanımlıyoruz.
  Serial.println(); 
  //Gelen datayı seri port üzerinden yolla.
  ...
  for(int i=0; i<readSize; i++){  //Gelen datayı diziye kaydet.
   ...
   if(payload[i] =='$'){
      rxBuffer[i]==payload[i+1];
      rxBuffer[i]==payload[i+2];
      i+=2;
    }
    if(payload[i] =='>'){
       rxBuffer[i]==payload[i+1];
  ...
  if( geldiMi==0){
      ...
      geldiMi=1;
        ...
        ikinciKisim[i]=payload[i];
    }
   */
   
/* if(payload[i] =='$'){
      ilkKisim[i+1]=payload[i+1];
     ilkKisim[i+2]=payload[i+2];
 ikinciKisim[i+4]=payload[i+4];
    ikinciKisim[i+5]=payload[i+5];
...
*/
 ...
 /* receivedData1= atoi(ilkKisim);
  receivedData2= atoi(ikinciKisim); //Buffer daki datayı int e çevir.
...
void loop() { // Sürekli döngü.
  ...
  client.loop();
 /*float ftemp2=atof(ikinciKisim);   
 float ftemp = atof(ilkKisim);
 
    pubMQTT(topic1, ftemp);
    pubMQTT(topic2, ftemp2);
    */
